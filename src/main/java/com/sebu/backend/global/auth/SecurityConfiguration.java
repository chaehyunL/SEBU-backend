...
public class SecurityConfiguration {
    ...
    private static final OAuth2Error INVALID_CLAIMS_ERROR = new OAuth2Error(
    ...
    BearerTokenResolver bearerTokenResolver() {
        ...
        return request -> isAuthApi(request) ? null : delegate.resolve(request);
    }

    private boolean isAuthApi(HttpServletRequest request) {
        String requestPath = request.getRequestURI().substring(request.getContextPath().length());
        return requestPath.equals(AUTH_API_PATH) || requestPath.startsWith(AUTH_API_PATH + "/");
    }

    @Bean
    SecretKey jwtSecretKey(TokenProperties properties) {
    ...
    private OAuth2TokenValidatorResult validateRequiredClaims(Jwt jwt) {
        if (jwt.getExpiresAt() == null || !Instant.now().isBefore(jwt.getExpiresAt())) {
            return OAuth2TokenValidatorResult.failure(EXPIRED_TOKEN_ERROR);
        }
        if (!"USER".equals(jwt.getClaimAsString("role")) || !isLong(jwt.getSubject())) {
            return OAuth2TokenValidatorResult.failure(INVALID_CLAIMS_ERROR);
        }
